...
Реализованы: фундамент, **Цель 1 «Купить ли вещь»** (с поиском дублей через эмбеддинги/pgvector)
и **Цель 2 «Что надеть»** (погода по геолокации + учёт трендов). Цель 3 (капсула + партнёрка) —
следующая (см. `REQUIREMENTS.md` §14).
